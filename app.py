...
# Connect Device - Logic moved to JS as server can't interact with Device directly.
...
async def decode_info(data: RawHexModel):
    ...
    try:
        ...
        return {"raw": data.raw_hex, "parsed": parsed, "status": "success"}
...
#Need to use this on a Disconnect Button. But need to analyse whether we really need a Disconnect as we scale to multiple devices.
# @app.post("/api/close-serial-port")
# async def close_serial_port():
#     close_serial_port(connection)


# Read Sensor Data. This can be discarded as we are now handling command preperation in JS and interacting with PORT directly from browser.
...
async def get_reading_history(db: Session = Depends(get_db)):
    ...
    return {
        "history": [
            {
                "timestamp": r.timestamp.astimezone(IST).strftime("%b %d %Y %H:%M:%S") ,#r.timestamp.strftime("%H:%M:%S"), # Format for chart labels
...
#This websocket is not required as we are not pooling device stream from devices default frquency but customized using user centric drop down.
...
#Set Correction value to calculate calibration factor A     
...
#set calibrations (manual/cancel manual/Range)
